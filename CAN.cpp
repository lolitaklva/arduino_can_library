...
void CAN::init(CAN& can_obj, uint8_t RX, uint8_t TX, uint32_t baud_rate) {
	...
	//	set OCR1B value
	OCR1B = OCR1A * 0.75;  // 75 % for bit sampling
	...
	if (!transmitting) {
		...
		if (shift < 57) {  // 57-bit frame
			...
			if (shift < 11) { // 11 bit identifier
				...
				if (tx_bit == rx_bit && shift == 41) { // check for ACK bit
					transmitting = false;
...
void CAN::error_frame() {
	...
	if (CAN::crc_error_flag || CAN::ack_error_flag || CAN::bit_stuffing_error_flag || CAN::form_error_flag || CAN::bit_error_flag || CAN::error_frame_flag) {
		
		global_error_flag = true; // global flag on
		
		if (ISR_dir) { // transmitter error handling logic
			if ((tx_error_counter < 127) || (tx_error_counter >= 127 && !CAN::ack_error_flag)) {
				tx_error_counter += 8;
			}
		...
		if (stat_error == 0){ // for error-active nodes
			...
			if (i < shift_error) { // for error-passive nodes
